...
describe('Focus after destructive actions (US3, keyboard)', () => {
  it('moves focus to Undo after Clear all, then to the list after Undo', async () => {
    const user = userEvent.setup()
    mockApi()
    render(<App />)
    await screen.findByText('1 USD = 0.8764 EUR')
  ...
  it('moves focus to the next row after deleting, and to the empty state when none remain', async () => {
    const user = userEvent.setup()
    mockApi()
    render(<App />)
    await screen.findByText('1 USD = 0.8764 EUR')
    ...
    await user.click(screen.getByRole('button', { name: 'Log conversion' }))
    await user.click(screen.getByRole('tab', { name: /Log/ }))
    const deletes = screen.getAllByRole('button', { name: /Delete conversion/ })
    deletes[0]!.focus()
    await user.keyboard('{Enter}')
    expect(screen.getByRole('button', { name: /Delete conversion/ })).toHaveFocus()
    await user.keyboard('{Enter}')
    expect(document.activeElement).not.toBe(document.body)
    expect(document.activeElement).toHaveTextContent('No conversions logged yet')
  })
})
